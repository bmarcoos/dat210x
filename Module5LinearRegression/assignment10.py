...
import os
folder_dir = os.path.dirname(os.path.realpath(__file__)) + r'/Datasets/recordings/'

for wav in os.listdir(folder_dir):        
    audio_file = wavfile.read(folder_dir + wav)

    zero.append(audio_file[1]) #data
...
X_test = test[0:round(Provided_Portion * n_audio_samples)]
...
X_train = train[:, 0:round(Provided_Portion * n_audio_samples)]
y_train = train[:, round(Provided_Portion * n_audio_samples):]
