...
def load(path_test, path_train):
  ...
  import math
  #desired 4%
  my_num =  int(math.ceil(X_train.shape[0] * 0.04))
  
  X_train = X_train.head(my_num)
  y_train = y_train[0:my_num]
...
model = SVC(kernel='rbf', C=1, gamma= 0.001) #linear, poly, rbf
...
fig = plt.figure()
plt.imshow(X_test.iloc[1000].reshape(8,8), cmap=plt.cm.gray_r, interpolation='nearest')
plt.title('1000 guess', fontsize=6)
plt.axis('off')
