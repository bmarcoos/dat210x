...
X = pd.read_csv('Datasets/breast-cancer-wisconsin.data', header= None, names= column_names)
...
feature_mean = X.mean()
X.replace(to_replace=['?','?','?','?','?','?','?','?'], value=feature_mean, inplace=True)
...
if Test_PCA:
  ...
  model = manifold.Isomap(n_neighbors=3, n_components=2)
...
X_train, X_test, label_train, label_test = train_test_split(X, status, test_size=0.33, random_state=7)
...
model = KNeighborsClassifier(n_neighbors = n, weights= 'distance') # uniform
...
print n, model.score(X_test, label_test)

plotDecisionBoundary(model, X_test, label_test)
